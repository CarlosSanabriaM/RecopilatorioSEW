...
<xsl:stylesheet version ="1.0" xmlns:xsl= "http://www.w3.org/1999/XSL/Transform">
    ...
    <!-- Se crea una variable con el ingrediente necesario -->
    <xsl:variable name="ingrediente_necesario" select="'pollo'"/>
    ...
    <xsl:template match="/">
        ...
        <html>
            ...
            <body>
                <header><h1 id="arriba">Recetas con pollo</h1></header>
                <main>
                    <!--  Iteramos por cada una de las recetas -->
                    <xsl:for-each select="recetas/receta">
                        <!-- Si la receta actual contiene entre sus ingredientes el pollo
                        entonces dicha receta aparecerá en el documento html de salida -->
                        <xsl:if test="contains(translate(ingredientes, 'P', 'p'),$ingrediente_necesario)">
                            <article>
